...
class GotoPoint():
    ...
    def __init__(self):
        ...
        linear_speed_robot = 10
        angular_speed_robot = 10
        ...
        while distance > 0.01:
            ...
            #tetrix 용 pub
            robot_odr.angular.z = angular_speed_robot* path_angle-rotation

            distance = sqrt(pow((goal_x - x_start), 2) +
                            pow((goal_y - y_start), 2))
            move_cmd.linear.x = min(linear_speed * distance, 0.1)
            #tetrix 용 pub
            robot_odr.linear.x = min(linear_speed_robot * distance, 1)

            if move_cmd.angular.z > 0:
                move_cmd.angular.z = min(move_cmd.angular.z, 1.5)

                #tetrix 용 pub
                robot_odr.angular.z = min(robot_odr.angular.z, 15)

            else:
                move_cmd.angular.z = max(move_cmd.angular.z, -1.5)

                #tetrix 용 pub
                robot_odr.angular.z = max(robot_odr.angular.z, -15)
